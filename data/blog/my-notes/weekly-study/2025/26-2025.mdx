...
这是一个基于 Next.js、Remotion 和 FFmpeg（WebAssembly 版）构建的在线视频编辑器项目，支持实时预览和高质量渲染。  
-
...
### 安装与使用  
- 📥 **桌面应用（macOS）**：下载最新版本并安装，仅支持 Apple Silicon 芯片（M1/M2/M3）。  
- 📦 **React 应用集成**：通过 `npm`、`yarn` 或 `pnpm` 安装依赖包，并配置 `useSyncQueriesExternal` 钩子。  
- 🔒 **生产环境安全**：自动禁用开发工具，避免生产环境泄露敏感信息。  

### 功能亮点  
- 🔄 **React Query 监控**：实时查看查询状态、缓存和失效情况。  
- 💾 **存储管理**：支持 MMKV、AsyncStorage 和 SecureStorage 的实时监控与操作。  
- 🌐 **环境变量跟踪**：方便调试不同环境配置。  
- 📱 **多平台兼容**：移动端、Web、桌面、TV 和 VR 均支持。  

### 常见问题  
- 🔌 **连接问题**：确保 DevTools 先启动，检查网络和端口配置。  
- 📱 **Android 设备连接**：使用真实设备时需设置 `isDevice: true`。  
- ⚠️ **设备 ID 持久性**：确保 `deviceId` 在应用重启后不变。  

### 未来计划  
- 🌐 **网络请求监控**：跟踪 API、WebSocket 和 GraphQL 请求。  
- 🧩 **插件系统**：支持社区扩展功能。  
- 🗄️ **Drizzle ORM 集成**：数据库管理工具支持。  
