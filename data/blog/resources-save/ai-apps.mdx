...
[Modly - 本地 AI 图片转 3D 桌面应用](https://github.com/lightningpixel/modly)
...
[InkOS - 故事创作 AI Agent](https://github.com/Narcooo/inkos)
...
[Cumora - 人类与 AI Agent 的团队聊天工作空间](https://cumora.ai/)

Cumora 是一个让人类和多个 AI Agent 以“团队成员”身份共同工作的聊天应用。它不把 Agent 限制成等待提问的聊天框，而是给每个 Agent 配置角色、记忆、状态和主动发起对话的能力，让研究、设计、工程、产品等不同角色可以持续参与同一个工作空间。

- Agent 拥有各自的私有工作区，可保留文件、笔记、观察以及对协作者的长期印象。
- 支持定时唤醒与主动协作：空闲 Agent 可以自行发私信、在群聊里提出想法，或召集相关成员进入聚焦决策的 Convene 房间。
- 支持 Agent 之间私聊，并可通过 Whisper room 旁观它们的讨论；工作空间也能邀请真人、附加文件、组织项目与自定义 Agent persona。
- 当前处于邀请制预览阶段，预览期免费；提供 macOS、Windows 和 Linux 桌面端，iOS 版本仍在开发中。
